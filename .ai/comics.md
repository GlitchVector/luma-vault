...
The panel's first step opens Claude Code on the comic (`ChatView`, topic `comic:<name>`, `/comic <name>`
prefilled): one Enter runs the `/comic` command, which asks its questions as numbered lists in the chat
