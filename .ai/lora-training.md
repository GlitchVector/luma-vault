...
| dataset viewer | the app's LoRAs page, "Training images" on a card: `lora_dataset` (`apps/desktop/src/lora.rs`) reads the dataset's `.toml` and shows every subset, repeat count and caption; the folder name per LoRA is `dataset` in `packages/core/src/loras.ts`. Above them, "Reference - not trained on": the owner's sheet(s) from `datasets/<name>/reference/`, which every prep copies from `sheets/<refs>-reference/` and no subset names (sheet text and palettes must never be trained) |
