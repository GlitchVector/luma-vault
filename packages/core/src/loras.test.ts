...
describe('one card per LoRA line', () => {
  it('lists variants under their parent, and gives a new line of the same character its own card', () => {
    ...
    expect(ari.map((card) => card.main.name)).toEqual(['ari_adopt_v4', 'ari_gen_s1'])
    ...
    expect(byStatus.wip.map((card) => card.main.name)).toContain('ari_gen_s1')
...
describe('adding an outfit', () => {
  const ari = CUSTOM_LORAS.find((entry) => entry.name === 'ari_gen_s1')!

  it('strips the version to find the line', () => {
    expect(loraLineBase('ari_gen_s1')).toBe('ari_gen')
  ...
  it('writes a message that starts the refs command and carries the names, the rules and the image', () => {
    ...
    expect(prompt).toContain("kind 'outfit', outfit 'Space Suit', parent 'ari_gen_s1'")
